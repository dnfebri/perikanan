<x-layouts-main-admin>
  ...
  <div class="py-10 max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
    ...
    <form action="{{ url('admin/gallery?id=') . $gallery['id'] }}" method="post" enctype="multipart/form-data">
      @csrf
      @method('put')
      <div class="grid md:grid-cols-2 md:gap-6 items-center">
        <div class="relative z-0 w-full mb-6 group">
          <input type="text" name="name" id="name" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" value="{{ $gallery['name'] }}" placeholder=" "/>
        ...
        <div class="relative z-0 w-full mb-6 group">
          ...
          <select id="columns" name="columns" class="border-0 border-b border-gray-300 text-gray-900 text-sm focus:ring-transparent focus:ring focus:ring-blue-500 focus:border-blue-500 block w-full dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            <option value="1" @if($gallery['columns'] == 1) selected @endif>1</option>
            <option value="2" @if($gallery['columns'] == 2) selected @endif>2</option>
          </select>
        </div>
      </div>
      <div class="grid md:grid-cols-2 md:gap-6">
        ...
        <div class="relative z-0 w-full mb-6 group">
          <img src="{{ url('images') . '/' . $gallery['image'] }}" class="h-32 img-previuw">
        </div>
      </div>
